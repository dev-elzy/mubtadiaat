      ...
      <div className="wrap">
        <div className="dual-layout" style={{ gridTemplateColumns: !showBerita ? '1fr' : undefined }}>
          ...
          <aside className="side-profil">
            ...
            <h1 className="hero-title" style={{ fontSize: '32px', textAlign: 'left' }} dangerouslySetInnerHTML={{ __html: heroTitleHtml }}></h1>
            <p className="hero-sub" style={{ textAlign: 'left', fontSize: '15px' }}>{heroSub}</p>
            
            <div className="hero-actions" style={{ justifyContent: 'flex-start', marginTop: '24px', flexWrap: 'wrap' }}>
              <a href={settings.daftarUrl || '#'} target="_blank" className="btn-primary" style={{ width: '100%', justifyContent: 'center' }}>Pendaftaran Online</a>
              <a href="/profil" className="btn-ghost" style={{ width: '100%', justifyContent: 'center' }}>Lihat Profil Lengkap</a>
            </div>
                ...
                <>
                  ...
                  <article className="mag-card" style={{ marginBottom: '24px' }}>
                    <a href={`/berita/${berita[0].slug}`} style={{ display: 'block', textDecoration: 'none' }}>
                      <img src={berita[0].image} alt={berita[0].title} style={{ width: '100%', height: '320px', objectFit: 'cover' }} />
